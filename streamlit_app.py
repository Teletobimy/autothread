...
st.write(
    "OpenAI GPT와 Threads Graph API를 활용해 지정한 간격으로 자동 게시합니다."
)
...
with st.sidebar:
    ...
    if not openai_key:
        st.warning(
            "Streamlit secrets 또는 환경 변수에 OPENAI_API_KEY 값을 설정해주세요.",
            icon="⚠️",
        )
...
topic = st.text_input(
    "게시 주제 (선택)", help="공백이면 GPT가 자유롭게 주제를 선택합니다."
)
...
if st.button("Threads에 게시 시작", type="primary", use_container_width=True):
    if not openai_key or not threads_token:
        st.stop()
    ...
    try:
        with st.spinner("게시 중입니다. 최대 몇 분이 걸릴 수 있습니다..."):
